...
"""
FAKE NEWS DETECTOR - FLASK API (PATTERN PIPELINE ONLY)
=======================================================
RESTful API using only the pattern detection pipeline
"""
...
# ------------------------------------------------------------
# LAZY MODEL INITIALIZATION (PATTERN PIPELINE ONLY)
# ------------------------------------------------------------
def load_models():
    global post_pattern_pipeline, models_ready
    if models_ready:
        return
    ...
    with init_lock:
        ...
        print("🚀 Initializing Pattern Pipeline (Single Model)...")
        from inference_post_patterns import ComprehensiveInferencePipeline

        post_pattern_pipeline = ComprehensiveInferencePipeline()
        # REMOVED: StandaloneFakeNewsDetector to save memory

        models_ready = True
        print("✅ Pattern pipeline ready (single model mode)")
...
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 8080))
    print(f"🚀 Server starting on port {port} (Pattern Pipeline Only)")
    app.run(host="0.0.0.0", port=port, debug=False, use_reloader=False)
